            ...
            idle: begin
                ...
                if(in_cu_valid && out_cu_ready) begin
                    case (in_cu_data[1:0])
                        ...
                        RD_TEXT:begin
                            n_total_bytes_left = (in_cu_data[5:4]  == aes_id) ? 16 : 32; // aes rd txt 16B sha rd txt 32B
                            n_opcode_q = FLASH_READ;
                            n_addr_q = out_address;
                            next_state = wip_poll_send; // ppll wip until not busy 
                            n_wip_poll_type = pp; // page programm 
                        ...
                        WR_RES: begin
                            n_total_bytes_left = (in_cu_data[3:2]== aes_id) ? 16 : 32; // aes wr txt 16B sha wr txt 32B
                            n_opcode_q = FLASH_PP;
                            n_addr_q = out_address;
                            next_state = wip_poll_send; // ppll wip until not busy 
                            n_wip_poll_type = pp; // page programm 
